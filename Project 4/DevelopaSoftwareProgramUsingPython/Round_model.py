...
class Round:
    ...
    def __init__(self, name: str = Any, list_of_finished_matches: list[Match] = Any):
    ...
    def run(self, sorted_player_list: list[Player], tournament):

        self.list_of_rounds = []
        self.list_of_finished_matches = []
        self.name = "Round " + str(len(tournament.list_of_rounds) + 1)
        ...
        for match in self.list_of_rounds:
            ...
            while not valid_score_player1:
                try:
                    score_player1 = float(input(f"Enter the score of {match.player_1} :"))
                except Exception:
                    print("You need to enter 0, 0.5 or 1")
                else:
                    match.score_player_1 = float(score_player1)
                    tournament.participant_score.append([match.player_1.id, match.score_player_1])
            ...
            while not valid_score_player2:
                try:
                    score_player2 = float(input(f"Enter the score of {match.player_2} :"))
                except Exception:
                    print("You need to enter 0, 0.5 or 1")
                else:
                    match.score_player_2 = float(score_player2)
                    tournament.participant_score.append([match.player_2.id, match.score_player_2])
